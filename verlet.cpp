...
namespace integrators
{
	...
	float verlet::nextPosition(float index, float pos[], float vel[], mathTools::points* pts, physics::forces* f)
	{
	...
	//Directly updates pts through the pointer.
	//Use float return for debugging if needed.
	float verlet::nextVelocity(float index, float pos[], float vel[], mathTools::points* pts, physics::forces* f)
	{
		float acc[3] = {0.0,0.0,0.0};
		f->getAcceleration(pos,vel,systemTime,acc);
		//Error code 0.0 -> No Error.
